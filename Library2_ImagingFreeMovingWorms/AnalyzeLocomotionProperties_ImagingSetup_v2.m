function [TrackLocomotionData, CurrentTrack] = AnalyzeLocomotionProperties_ImagingSetup_v2(CurrentTrack, Settings)
%
% September 2019, Sagi Levy
%
%%
% This function will put NaN values where ever there are too many sequential frames missing.   
...
% NOTE THIS IS NOT ACCELERATION: 
%  acc = d(|V(t)|*U(t))/dt = d(|V(t)|)/dt * U(t) +  d(U(t))/dt* |V(t)| 
%  where |V(t)|=speed, U(t)=unit vector in the speed tangent direction  
...
% Jerk  
% What is called here Jerk Amplitude is the derivative of the amplitude component of 'ChangesIn_Smoothed_Velocity)'.    
% Although it has units of degrees/frames^3 it is NOT Jerk per se!
%    Consider using another smoothing here prior to differentiation (since the smoothing is cubic while here I am analyzing a 3rd derivative). 
%    x                                               = 1:length(ChangesIn_Smoothed_Velocity.Amplitude);
%    doubleSmoothed_ChangeInVelocityAmplitude        = single(csaps(x, double(ChangesIn_Smoothed_Velocity.Amplitude), InterpolationFactor, x));
...
%% Plots
PlotTrackLocomotion(TrackLocomotionData, coordinates, PlotInfo) ;
