function [BehaviorCode, Segments_HighLevelBehavior, Segments_LowLevelBehavior, Segments] = SegmentBehavior_SingleTrack_HighThroughputDevice_02(CurrentTrack, tr_num, Settings)
%
...
if DirectionDuringRunsIsDefined
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
    %%  Low level Behavior segmentation Strategy 
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
    ...
    %
    % Notes: Concatinate straight runs that are: proximal, without pause/omega/(sharp turns), same direction.   
    %        Correct the omegas and pauses states acordingly. 
    %        Define curving frames within the straight runs.
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ~isempty(Segments.OutOfBounds)    
        Segments.OutOfBounds                                     = ExtendOutOfBound  (Segments.OutOfBounds,  HighVelocity, Thresholds.StraightRun.MinFramesForShortRun);
    ...
    % Omega state: a long omega shape segment coupled with a long low velocity segment    
...
function SegmentsOutOfBound  = ExtendOutOfBound (SegmentsOutOfBound, HighVelocity, MinFramesForShortRun)
%% Concatinate proximal Out-of-bounds segments if there is more than 'MinFramesForShortRun' of HighVelocity frames within the interval between segments 
if isempty(SegmentsOutOfBound)
    return    
end
    
AllOutOfBoundFrames_logical                                     = false(1,length(HighVelocity));
AllOutOfBoundFrames_logical([SegmentsOutOfBound.FramesIndices]) = true;
Frames_Not_Allowed_For_Extension                                = HighVelocity;   % only if there is more than 'MinFramesForShortRun' of HighVelocity frames within the interval between segments  
...
for segment_index = 2:length(SegmentsOutOfBound)
    ...
    if ~isempty(FramesInInterval)
        Criterion_For_Concatination = ( sum(Frames_Not_Allowed_For_Extension(FramesInInterval))<MinFramesForShortRun );          
        if Criterion_For_Concatination
...
%% This function will generate new fields: 'Field1__ExtendedBy_Field2'
% Inputs:
%   SegmentsIn        =  Segment structure
%   Field1 and Field2 = strings with the name of fields
...
if TotalFramesCount~=NumberOfFrames
...
% HighLevel_CodeName   = {'Out of bounds','Forward',          'Curve',         'Pause','Reversal','Omega-Pause',          'SharpTurn', 'Pirouette. Until pause', 'Pirouette. After pause'};
% HighLevel_FieldNames = {'OutOfBounds'  ,'Forward_NoCurving','CurvingForward','Pause','Reversal','OmegaWithoutPirouette','SharpTurns','Pirouette_UntilPause',   'Pirouette_AfterPause'};
HighLevel_CodeNumber = [      0             1                  2               3       4              5                     6                  7                           8            ];     
...
% Functions not used
function Add_Ylines(f, Frames, ChangeTicks, COLOR_IN, LineStyle_IN)  % Add vertical lines to ALL subplots in the figure
% f           = figure handle
% Frames      = frames where vertical lines will be added
% ChangeTicks = optional switch... 
%                0 = no 
%                1 = use only 'Frames' and the current xlimits as xticks
%                2 = add 'Frames' to the current xticks
% COLOR       = optional color of lines

if exist('COLOR_IN','var')
    COLOR = COLOR_IN;
else
    COLOR = 'r';
end
if exist('LineStyle_IN','var')
    LineStyle = LineStyle_IN;
else
    LineStyle = ':';
end

children_h = get(f,'children'); 
for h_ind = 1:length(children_h)
    h = children_h(h_ind);
    ylimits = get(h,'Ylim');
    for F = Frames
        line([F F],ylimits,'color',COLOR,'linestyle',LineStyle,'parent',h); hold on;
        figure(f);
    end
    if exist('ChangeTicks','var')
        ...
        switch ChangeTicks
            case 1
                Ticks = [Ticks(1) Frames  Ticks(end)];
                set(h,'xtick',Ticks);
            case 2
                Ticks = unique(sort ([ Ticks Frames]));
                set(h,'xtick',Ticks);
        end
    end
end
